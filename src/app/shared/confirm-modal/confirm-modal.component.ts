    ...
    <app-modal [isOpen]="isOpen()" (modalClose)="close.emit()" [maxWidth]="'sm'">
      ...
      <div modal-footer>
        ...
        <button (click)="confirm.emit()" 
                [class]="confirmClass()"
                class="w-full sm:w-auto px-5 py-2.5 rounded-xl font-semibold text-white shadow-neumorphic-raised hover:shadow-neumorphic-pressed active:shadow-neumorphic-sunken transition-all">
          {{ confirmText() }}
        </button>
      </div>
    </app-modal>
